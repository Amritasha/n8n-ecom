...
if [ ! -f "$IMPORTED_FLAG" ]; then
  ...
  until curl -sf "http://localhost:$N8N_PORT/healthz" > /dev/null 2>&1; do
    ATTEMPT=$((ATTEMPT + 1))
    echo "[bundle] Health check attempt $ATTEMPT failed, retrying in 3s..."
    sleep 3
  done
  ...
  until curl -sf "http://localhost:$N8N_PORT/rest/settings" | grep -q 'isInstanceOwnerSetUp.*true'; do
    ATTEMPT=$((ATTEMPT + 1))
    OWNER_STATUS=$(curl -sf "http://localhost:$N8N_PORT/rest/settings" 2>/dev/null | grep -o 'isInstanceOwnerSetUp[^,}]*' || echo "unknown")
